...
- N/A — Platform Engineering page content is a static local TypeScript content module, no CMS/API integration this phase, by clarification (TMS-86-platform-engineering)

## Recent Changes
- TMS-86-platform-engineering: planning the Platform Engineering page at the new route `/what-we-do/platform-engineering` (`app/what-we-do/platform-engineering/`) — the fourth page in the "What We Do" family, spec directory disambiguated from `specs/TMS-86/`, `specs/TMS-86-software-product-engineering/`, and `specs/TMS-86-data-and-ai-engineering/`. All three sibling pages have since been upgraded to a live CMS integration in their own separate, later tickets; this ticket deliberately targets their *original* static-content shape instead, confirmed via `/speckit.clarify`. Introduces **zero** new shared `components/ui/` primitives, tokens, or icons — reuses `Hero`, `ContentBlock`, `GlassCard` (`serviceCapability` variant), `ProcessSteps`, `Faq`, `IconTile`, `final-cta`, and `MediaSlot` unmodified; all 16 icon slots this page needs (6 "why" tiles, 4 platform/industry cards, 6 related-service links) resolve to existing `components/ui/icons.tsx` entries, including reusing `EradicateDebtIcon` twice on the same page (once for the "Accelerate delivery" why-tile, once — per established precedent — for the "Software Product Engineering" related-service link). Hero right-side card uses the `public/samples/svc-uiux.png` image (the last remaining unused `svc-*` asset after the software-product-engineering and data-ai-engineering siblings claimed `svc-eng.png`/`svc-qa.png`) in place of the reference's four stat-tile callouts, dropping the caption line entirely (per clarification, matching the Software Product Engineering sibling's treatment rather than the other two siblings' kept-caption one). One genuine content/structure difference preserved rather than normalized: the "Platforms for every stage of growth" grid mixes two non-clickable generic cards (SaaS Platforms, Enterprise Apps) with two clickable industry cards (HealthTech, FinTech) — modeled as an optional `href` field reusing the Data & AI Engineering sibling's already-shipped conditional-`<Link>`-wrap pattern, not a new component. Two 1-line nav-config edits repoint "Platform Engineering" from `/services#svc-platform` to the new route: `cms/api/footer.ts` and `cms/api/header.ts`'s `toMegaGroup()` special-case ternary (extending the existing three-entry chain to a fourth).

## Active Technologies
- TypeScript 5 (strict mode, per `tsconfig.json`) + Next.js 16.2.10 (App Router), React 19.2.4, Tailwind CSS v4 (TMS-86-ai-strategy-and-roadmap)
- N/A — AI Strategy & Roadmap page content is a static local TypeScript content module, no CMS/API integration this phase, by clarification (TMS-86-ai-strategy-and-roadmap)

## Recent Changes
- TMS-86-ai-strategy-and-roadmap: planning the AI Strategy & Roadmap page at the new route `/what-we-do/ai-strategy-roadmap` (`app/what-we-do/ai-strategy-roadmap/`) — the sixth (and CTO-advisory-shaped) page in the "What We Do" family, spec directory disambiguated from the five earlier `specs/TMS-86*` siblings. Like Platform Engineering and Managed Services, this ticket deliberately targets the static-content build phase (confirmed via `/speckit.clarify`) even though all five sibling pages have since been upgraded to live CMS integration in their own separate, later tickets. Introduces **zero** new shared `components/ui/` primitives or tokens — reuses `Hero`, `ContentBlock`, `GlassCard` (`serviceCapability` variant, confirmed icon-less for capability cards), `ProcessSteps` (default `columns=5`, unchanged), `Faq`, `IconTile`, and `final-cta` unmodified. Of 16 icon slots (6 "why" tiles, 4 advisory-segment cards, 6 related-service links), 14 resolve to existing `components/ui/icons.tsx` entries (including reusing `SvcStrategyIcon` — this page's own nav icon — for its "AI-native leadership" why-tile, and near-exact reuses of `HamburgerIcon`/`ConstructionIcon` for PE/VC Portfolio/Enterprise Programs); the remaining 2 slots (a single-person silhouette needed twice, with no existing equivalent — `UsersIcon` is a distinct two-person shape) justify one new `UserIcon` export added to the shared icon file. Unlike Platform Engineering/Data & AI Engineering's mixed link/no-link industry grids, this page's "Founders. Boards. Scaling technology orgs." cards are **all four** non-interactive (no `href` field modeled at all — a genuine, simpler reference fact, not a normalization). Hero right-side card uses `public/samples/dm-scalability.png` (per clarification, following the five built siblings' image-replacement precedent) in place of the reference's four stat-tile callouts (Advisors/Engagement/Roadmap/Outcomes) and its "AI IMPACT™ · 4D™ · PRISM™ frameworks" caption line, both dropped entirely. One new `components/ui/ambient-orbs.tsx` pathname branch reuses four *already-existing* color tokens (`--color-overlay-orange`, `-amber-12`, `-orange-10`, `-orange-11`) in a combination no prior branch happens to use — zero new tokens, simpler than Managed Services' own ambient-orb decision (which needed one genuinely new token). Two 1-line nav-config edits repoint "AI Strategy & Roadmap" from `/services#svc-strategy` to the new route: `cms/api/footer.ts` and `cms/api/header.ts`'s `toMegaGroup()` special-case ternary (extending the existing five-entry chain to a sixth). CTA links use the app's real `/contact-us` route (not the placeholder `/contact` some earlier sibling docs referenced) and the now-built `/how-we-work/discovery-sprints` route for "Explore Discovery Sprints".
