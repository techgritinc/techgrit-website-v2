...
function WhyTile({ tile }: { tile: ValuePropositionTile }) {
  return (
    <div className="flex gap-4 rounded-xl border border-border-8 bg-glass-3 p-6">
      ...
      <div>
        <h4 className="text-[16.5px] leading-[normal] tracking-[normal] font-bold text-white">{tile.title}</h4>
...
export function AiStrategyRoadmapWhy({ section }: { section: WhySection }) {
  return (
    <section className="relative">
      <div className="mx-auto max-w-[1280px] px-9 py-[60px]">
        <RevealOnScroll>
          ...
          {/* Collapses at 921px, matching the reference's [data-why-grid] @media(max-width:920px)
              rule verbatim — not this project's canonical md=960px breakpoint. */}
          <div className="grid grid-cols-1 gap-4 min-[921px]:grid-cols-2">
            {section.tiles.map((tile) => (
              <WhyTile key={tile.id} tile={tile} />
            ))}
          </div>
        </RevealOnScroll>
      </div>
