...
export default function PlatformSection({ data }: { data: DeliveryEngineData }) {
  const { badgeLabel, title, subtitle, capabilities, image, dashboard } = data;
  const hasDashboard = dashboard.metrics.length > 0 || dashboard.pipeline.length > 0;
      ...
      <div className="mx-auto grid max-w-(--container-max) grid-cols-[1fr_1.05fr] items-center gap-tg-20 px-9 py-[96px] max-tg-md:py-[76px] max-tg-sm:py-[56px] max-tg-md:grid-cols-1 max-tg-md:gap-tg-17">
        ...
        <div className="relative">
          ...
          {hasDashboard ? (
            ...
            <div className="relative z-1 aspect-[4/3] overflow-hidden rounded-xl border border-border shadow-[var(--shadow-console-card)] backdrop-blur-lg">
              <MediaSlot src={image?.url} alt={image?.alt ?? title} fill sizes="(max-width: 960px) 100vw, 50vw" />
            </div>
          )}
        </div>
      </div>
