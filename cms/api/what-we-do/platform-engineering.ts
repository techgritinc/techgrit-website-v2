...
import type {
  ...
  OutcomeSection,
  PlatformEngineeringPageContent,
  PlatformEngineeringSection,
  SectionImage,
  StrategiesSection,
  ValuePropositionTile,
  WhySection,
} from "@/app/what-we-do/platform-engineering/_data/types";

const PLATFORM_ENGINEERING_ENDPOINT = "/api/pages/by-slug/platform-engineering";
...
function toHeroSection(section: StrapiPlatformEngineeringHeroSection, order: number): HeroSection {
  return {
    type: "hero",
    order,
...
// This page's own occurrence has shipped `blockers: null` — same nullable handling as
// both sibling "What We Do" pages' own occurrences of this shared component.
function toIntroSection(section: StrapiPlatformEngineeringChallengesSection, order: number): IntroSection {
  return {
    type: "intro",
    order,
...
// falls back to empty rather than fabricating a description or eyebrow label.
function toCapabilitiesSection(section: StrapiPlatformEngineeringCapabilitiesSection, order: number): CapabilitiesSection {
...
function toServiceDetailSection(section: StrapiServiceDetailSection, order: number): PlatformEngineeringSection | null {
...
function toFaqSection(section: StrapiPlatformEngineeringFaqSection, order: number): FaqSection {
  return {
    type: "faq",
    order,
...
// data, just the other field on the same response — never a hardcoded destination; only
// renders a secondary button when the CMS has actually labeled one.
function toCtaSection(section: StrapiPlatformEngineeringCtaBannerSection, order: number): FinalCtaSection {
...
function toSection(raw: StrapiPlatformEngineeringSection, order: number): PlatformEngineeringSection | null {
...
// Called directly from the (async) Platform Engineering Server Component
// (await getPlatformEngineeringData()) — runs on the server for every request, so CMS
// edits show up on the next page load with no rebuild. Mirrors both sibling "What We Do"
...
// fallback content: getPlatformEngineeringData() returns null when the CMS is
// unreachable or returns no usable sections, and the page itself calls notFound() in
// that case.
export async function getPlatformEngineeringData(): Promise<PlatformEngineeringPageContent | null> {
  const data = await fetchCms<StrapiPlatformEngineeringPage>(PLATFORM_ENGINEERING_ENDPOINT);
  if (!data) return null;

  const rawSections = data.sections ?? [];
  const sections = rawSections
    .map((raw, index) => toSection(raw, index + 1))
    .filter((section): section is PlatformEngineeringSection => section !== null);
