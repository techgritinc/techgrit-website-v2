...
2. **Create feature directory and spec file**:

   a. Check existing specs directories to determine the next available number:
      - Specs directories: Check for directories matching `specs/[0-9]+-*`
      - Find the highest number N, use N+1
   ...
   **IMPORTANT**:
   - If no existing directories found, start with number 1
   - You must only ever run this script once per feature
   - The JSON is provided in the terminal as output - always refer to it to get the actual content you're looking for
   - The JSON output will contain BRANCH_NAME (used as directory name) and SPEC_FILE paths
   - For single quotes in args like "I'm Groot", use escape syntax: e.g 'I'\''m Groot' (or double-quote if possible: "I'm Groot")
   - **No git branch is created at this stage.** Branch creation happens during `/speckit.implement`.
...
8. Report completion with feature directory name, spec file path, checklist results, and readiness for the next phase.
   ...
   **MANUAL REVIEW GATE**: After spec generation, the workflow pauses for human review. Display:
   ...
   The specification is ready for human review before proceeding to planning.

   **Spec file**: <SPEC_FILE path>

   ### Next steps:
   1. Review the spec.md and verify it captures the intended feature
   2. (Optional) Run `/speckit.clarify` to resolve ambiguities
   3. Once satisfied, run `/speckit.plan` to generate the technical plan and tasks
   ---
   ```

**NOTE:** The script creates the feature directory and initializes the spec file before writing. No git branch is created at this stage.
